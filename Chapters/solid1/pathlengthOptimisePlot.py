...
for i in range(1, g1.GetXaxis().GetNbins() + 1):
	xs.append(500*g1.GetXaxis().GetBinCenter(i))
...
ysFit = []
for x in xsFit: ysFit.append(g3.GetFunction("gaus").Eval(x/500.))
...
plt.ylabel("N")
plt.xlabel("Pathlength Residual (mm)")
s = "$\sigma_{fit} = " + str(round(500*g3.GetFunction("gaus").GetParameter(2), 2)) + "\pm" + str(round(500*g3.GetFunction("gaus").GetParError(2), 2)) + " mm$"
plt.text(-100*1.85, 2050, s, fontsize = 16)
